...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getKnowledgeBaseArticles() {
    const data = await prisma.knowledge_base_article
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'knowledge_base_article'));
  ...
  async function createKnowledgeBaseArticle() {
    ...
    const data = await prisma.knowledge_base_article.create({
      data: body,
    });
    return res.status(200).json(data);
  }
